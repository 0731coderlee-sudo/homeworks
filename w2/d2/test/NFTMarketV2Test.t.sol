...
contract NFTMarketV2Test is Test {
    ...
    uint96 public constant TTC_PRICE = 1000 * 10**18;
    uint96 public constant USDC_PRICE = 100 * 10**18;
    uint96 public constant DAI_PRICE = 500 * 10**18;
    ...
    function setUp() public {
        ...
        market = new NFTMarketV2(ttcToken);
    ...
    function testAddSupportedTokens() public {
        // 只有owner可以添加代币 - V2使用custom error
        vm.expectRevert(NFTMarketV2.NotOwner.selector);
        vm.prank(seller);
        ...
        emit NFTMarketV2.TokenAdded(address(usdcToken));
        market.addSupportedToken(address(usdcToken));

        vm.expectEmit(true, false, false, false);
        emit NFTMarketV2.TokenAdded(address(daiToken));
    ...
    function testListWithDifferentTokens() public {
        ...
        emit NFTMarketV2.Listed(address(nft), TOKEN_ID_1, seller, TTC_PRICE, address(ttcToken));
        market.listWithToken(address(nft), TOKEN_ID_1, TTC_PRICE, address(ttcToken));

        vm.expectEmit(true, false, false, true);
        emit NFTMarketV2.Listed(address(nft), TOKEN_ID_2, seller, USDC_PRICE, address(usdcToken));
        market.listWithToken(address(nft), TOKEN_ID_2, USDC_PRICE, address(usdcToken));

        // 使用默认ttcoin上架
        vm.expectEmit(true, false, false, true);
        emit NFTMarketV2.Listed(address(nft), TOKEN_ID_3, seller, DAI_PRICE, address(ttcToken));
        market.list(address(nft), TOKEN_ID_3, DAI_PRICE);

        vm.stopPrank();

        // 验证上架信息
        (address seller1, uint96 price1, address token1) = market.getListing(address(nft), TOKEN_ID_1);
        assertEq(seller1, seller);
        assertEq(price1, TTC_PRICE);
        assertEq(token1, address(ttcToken));

        (address seller2, uint96 price2, address token2) = market.getListing(address(nft), TOKEN_ID_2);
    ...
    function testBuyWithSpecificToken() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit NFTMarketV2.Bought(address(nft), TOKEN_ID_1, buyer1, USDC_PRICE, address(usdcToken));
        ...
        // 验证上架记录已清除
        (address clearedSeller, uint96 clearedPrice, address clearedToken) = market.getListing(address(nft), TOKEN_ID_1);
    ...
    function testCallbackBuyOnlyTTCoin() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit NFTMarketV2.Bought(address(nft), TOKEN_ID_1, buyer1, TTC_PRICE, address(ttcToken));
    ...
    function testCallbackPaymentTokenMismatch() public {
        ...
        vm.expectRevert(NFTMarketV2.PaymentTokenMismatch.selector);
        vm.prank(buyer1);
        ttcToken.transferWithCallback(address(market), TTC_PRICE, data);
    }
    ...
    function testRemoveSupportedToken() public {
        ...
        // 移除USDC支持
        vm.expectEmit(true, false, false, false);
        emit NFTMarketV2.TokenRemoved(address(usdcToken));
        ...
        vm.expectRevert(NFTMarketV2.TokenNotSupported.selector);
        market.listWithToken(address(nft), TOKEN_ID_1, USDC_PRICE, address(usdcToken));
        vm.stopPrank();
    }
    ...
    function testListingSuccessAndFailures() public {
        ...
        vm.startPrank(seller);
        nft.approve(address(market), TOKEN_ID_2);
        vm.expectRevert(NFTMarketV2.PriceZero.selector);
    ...
    function testBuyEdgeCases() public {
        ...
        vm.startPrank(buyer2);
        usdcToken.approve(address(market), USDC_PRICE);
        vm.expectRevert(NFTMarketV2.NotListed.selector);
        market.buyNFT(address(nft), TOKEN_ID_1);
        vm.stopPrank();
        ...
        vm.prank(buyer1);
        vm.expectRevert(NFTMarketV2.NotEnoughPayment.selector);
        ...
        vm.prank(buyer1);
        vm.expectEmit(true, false, false, true);
        emit NFTMarketV2.Bought(address(nft), TOKEN_ID_2, buyer1, TTC_PRICE, address(ttcToken));
