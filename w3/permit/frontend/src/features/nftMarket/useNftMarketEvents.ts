...
export interface PermitBoughtEvent extends MarketEventBase { type: 'PermitBought'; buyer: `0x${string}`; }
export type MarketEvent = ListedEvent | BoughtEvent | PermitBoughtEvent;
...
export function useNftMarketEvents(limit = 50) {
  ...
  const span = 10n; // 仅回溯最近 10 个区块
        ...
        const permitBoughtFragment = (nftMarketAbi as any[]).find(i => i.type==='event' && i.name==='PermitBought');
        if (!listedFragment && !boughtFragment && !permitBoughtFragment) {
  ...
  let batchSize = 10n; // 初始批量（不超过 span）
        ...
        while (cursor <= latest) {
          ...
          while (!attemptSuccess && safetyRetry < 10) {
            ...
            try {
              const [listedLogs, boughtLogs, permitBoughtLogs] = await Promise.all([
                ...
                permitBoughtFragment ? publicClient.getLogs({ address: NFT_MARKET_ADDRESS, events: [permitBoughtFragment], fromBlock: cursor, toBlock: to }) : Promise.resolve([])
              ]);
              // 处理结果
              ...
              for (const l of permitBoughtLogs) {
                const anyLog: any = l as any;
                hist.push({
                  type: 'PermitBought',
                  nft: anyLog.args.nft,
  ...
  useEffect(() => {
    ...
    const unwatchPermitBought = publicClient.watchContractEvent({
      address: NFT_MARKET_ADDRESS,
      abi: nftMarketAbi,
      eventName: 'PermitBought',
      onLogs: logs => {
        const mapped: MarketEvent[] = logs.map(l => {
          const anyLog: any = l as any;
          const evt: PermitBoughtEvent = {
            type: 'PermitBought',
            nft: anyLog.args.nft,
            tokenId: anyLog.args.tokenId,
            buyer: anyLog.args.buyer,
          ...
          console.log('[NFTMarket][PermitBought]', evt);
          return evt;
        });
        if (mapped.length) addEvents(mapped);
      },
      onError: (e) => console.debug('watch PermitBought error', e)
    });

    return () => { unwatchListed?.(); unwatchBought?.(); unwatchPermitBought?.(); };
  }, [addEvents]);
